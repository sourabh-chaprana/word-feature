...
if (!fs.existsSync(sslKeyPath) || !fs.existsSync(sslCertPath)) {
  console.error('❌ SSL certificates not found!')
  console.error('   Run: npm run generate-ssl')
  process.exit(1)
}
...
if (fs.existsSync(frontendDistPath)) {
  ...
  console.warn('⚠️  dist folder missing. Build frontend: cd ../vite-project && npm run build')
}
...
app.get('/taskpane.html', (req, res) => {
  const distTaskpane = path.join(frontendDistPath, 'taskpane.html')
  const publicTaskpane = path.join(publicPath, 'taskpane.html')

  if (fs.existsSync(distTaskpane)) {
    res.sendFile(distTaskpane)
  } else if (fs.existsSync(publicTaskpane)) {
    res.sendFile(publicTaskpane)
  } else {
    res.status(404).send('taskpane.html not found. Build the frontend first.')
  }
})
...
server.listen(PORT, () => {
  ...
  console.log(`   Taskpane: https://localhost:${PORT}/taskpane.html`)
  console.log(`   Manifest: https://localhost:${PORT}/manifest.xml`)
  console.log('\n⚠️  Self-signed cert: accept the browser warning to continue.\n')
})
